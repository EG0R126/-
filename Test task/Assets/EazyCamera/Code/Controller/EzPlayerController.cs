...
namespace EazyCamera.Legacy
{
    ...
    public class EzPlayerController : MonoBehaviour
    {
        ...
        [SerializeField] private float speedMultiplier = 1.0f; // Коэффициент скорости камеры
...
#else
                HandleLegacyInput();
                transform.Translate(new Vector3(movement.x * speedMultiplier * Time.deltaTime, movement.y * speedMultiplier * Time.deltaTime, movement.z * speedMultiplier * Time.deltaTime), Space.World);
#endif // ENABLE_INPUT_SYSTEM
            }
